...
at fault. In **nine of the fourteen, the finding survived and the instrument
was what broke** — which is the pattern worth knowing about this project.
Rows 13 and 14 are the sharpest: the harness built to catch this class of error
had the same class of error in it, and then row 13's *fix* had it too.
...
| 14 | Row 13's fix handles the machines that cannot run a gate | `except ImportError` missed a torch that is **installed but broken** — a mismatched CUDA driver raises `OSError` from `_load_global_deps` and never reaches an `ImportError` handler, so the gate said FAIL and "do not ship" on an ordinary workstation with stale drivers. `reportlab` was unguarded entirely | **The guard was narrower than the failure it guards against** — row 13's own fix, one level down. Now `(ImportError, OSError)` on both, with the exception type printed so a real defect stays visible | `test_equivalence.py`, `build_summary_pdf.py` |

**Rows 1, 3, 6, 7, 10, 11, 12, 13 and 14 are instrument failures** — the
science was fine and the thing measuring it was not. Rows 2, 5, 8 and 9 are
hypotheses the data refuted. Row 4 is a retraction that was itself wrong.
...
And its corollary, learned a third time (row 14): **the guard is usually
narrower than the failure it guards against.** `ImportError` does not cover a
broken install; a 404 check does not cover a proxy. When row 14 was found we
audited every gate for the same shape rather than patching the one instance,
which is how the unguarded `reportlab` import surfaced before it ever bit.
